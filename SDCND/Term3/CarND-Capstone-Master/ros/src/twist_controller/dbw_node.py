...
class DBWNode(object):
    ...
    def loop(self):
        rate = rospy.Rate(50) # 50Hz #it was 50 orig and 20 before 12oct
        while not rospy.is_shutdown():
            if (self.final_waypoints is not None) and (self.current_pose is not None) \
            and (self.current_velocity is not None) and (self.target_velocity is not None):
                ...
                sample_step = ros_duration.secs + (1e-9 * ros_duration.nsecs)

                self.previous_loop_time = current_time

                throttle, brake, steering = self.controller.control(current_pose, target_velocity, current_velocity, final_waypoints, self.dbw_enabled , sample_step)
